...
print("R is reading")
ngl.abund.clusters.cast <- readRDS("R_script/ngl.abund.clusters.cast.vog.RDS")
mgCST.hclust <- readRDS("R_script/mgCST.hclust.vog.RDS")
mgCST.dist <- readRDS("R_script/mgCST.dist.vog.RDS")

relabund<-ngl.abund.clusters.cast/rowSums(ngl.abund.clusters.cast)

mgCSTs.sort.df <- read_csv("Data/vog_mgCST_sort_color.csv")
mgCSTs.samples.df <- read_csv("Data/vog_mgCST_samples_color.csv")
subspecies.with.colors <- read_csv("Data/vog_subspecies_with_colors.csv")
...
pdf("Medias/mgCST_VOG_heatmap.pdf", width=10, height=10)
